...
type PopupProps = {
  ...
  onEscape?: () => void;
  onDoubleClick?: () => void;
  onDrag?: (nextViewer: JSX.Element) => void;
  setMainViewer: (nextViewer: JSX.Element) => void;
};
...
export const PortalPopup: FunctionComponent<PopupProps> = ({
  ...
  const ondraggable = useCallback(
    (e: React.MouseEvent<HTMLDivElement>) => {
      if (onclick && (e.target as HTMLElement).classList.contains(styles.draggable)) {
        setMainViewer(<>{children}</>);
      }
    },
    [onDrag]
  );
  ...
  return (
    <Portal>
      ...
      >
        <div className={styles.draggable} onClick={ondraggable} onDrag={ondraggable}>
          {children}
      </div>
      </div>
    </Portal>
